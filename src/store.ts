...
export class Store<T extends object> {
  ...
  /**
   * Flag that defines whether the store
   * should mutate the state object on updates.
   */
  isMutateState: boolean;
   ...
   */
  constructor(state: T, isMutateState: boolean = false) {
    ...
    this.keys.forEach((key) => {
      ...
      if (isMutateState) {
        this.values[key].addListener((_, value) => {
          this.state[key] = value as T[keyof T];
        }, false);
      }
    });
    ...
    this.isStateActual = this.isStateActual.bind(this);
  ...
  isStateActual(): boolean {
    for (const key of this.keys) {
      if (this.values[key].value !== this.state[key]) {
        return false;
      }
    }
    return true;
...
export const createStore = <T extends object>(state: T, isMutateState?: boolean) => {
  return new Store<T>(state, isMutateState);
};
